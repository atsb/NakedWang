...
SpawnShrap(short ParentNum, short Secondary)
    ...
    long shrap_floor_dist = Z(2);
    long shrap_ceiling_dist = Z(2);
    long shrap_z = 0;
    long nx,ny;
    ...
    if (TEST(parent->extra, SPRX_BREAKABLE))
        {
        // if no user
...
WeaponMoveHit(short SpriteNum)
    ...
    case HIT_WALL:
            ...
            hitscan(sp->x, sp->y, sp->z, sp->sectnum,   // Start position
...
int
DoHitscanDamage(short Weapon, short hitsprite)
    {
    SPRITEp wp = &sprite[Weapon];
    USERp wu = User[Weapon];
    short stat;
...
DoStar(SHORT Weapon)
    ...
    USERp u = User[Weapon];
    LONG enemy, next_enemy, dax, day, daz, nextj;
    ...
    if (u->WpnGoal == -1)
        ...
        if (u->WaitTics > delay_tics)
            ...
            if (TEST(u->Flags2, SPR2_DONT_TARGET_OWNER))
                {
                if ((hitsprite = PickEnemyTarget(sp, aware_range)) != -1)
                    {
...
VectorMissileSeek(SHORT Weapon, SHORT delay_tics, SHORT turn_speed, SHORT aware_range1, SHORT aware_range2)    
    ...
    if (u->WpnGoal == -1)
        ...
        if (u->WaitTics > delay_tics)
            ...
            if (TEST(u->Flags2, SPR2_DONT_TARGET_OWNER))
                {
                if ((hitsprite = PickEnemyTarget(sp, aware_range1)) != -1)
                    {
                    SPRITEp hp = &sprite[hitsprite];
                    USERp hu = User[hitsprite];
...
int
SpawnExtraMicroMini(SHORT Weapon)
    {
    SPRITEp sp = &sprite[Weapon];
    USERp u = User[Weapon];
    SPRITEp wp;
...
int
SpawnBreakFlames(SHORT SpriteNum, SHORT enemy)
    {
    SPRITEp sp = &sprite[SpriteNum];
    USERp u = User[SpriteNum];
    SPRITEp ep = &sprite[enemy];
...
int
SpawnGoroFireballExp(SHORT Weapon)
    {
    SPRITEp sp = &sprite[Weapon];
    USERp u = User[Weapon];
    SPRITEp exp;
...
int
SpawnNuclearSecondaryExp(SHORT Weapon, short ang)
    {
    SPRITEp sp = &sprite[Weapon];
    USERp u = User[Weapon];
    SPRITEp exp;
...
int
SpawnGrenadeSecondaryExp(SHORT Weapon, short ang)
    {
    SPRITEp sp = &sprite[Weapon];
    USERp u = User[Weapon];
    SPRITEp exp;
...
DoFindGround(SHORT SpriteNum)
    ...
    save_cstat = sp->cstat;
    RESET(sp->cstat, CSTAT_SPRITE_BLOCK | CSTAT_SPRITE_BLOCK_HITSCAN);
    FAFgetzrange(sp->x, sp->y, sp->z, sp->sectnum, &u->hiz, &ceilhit, &u->loz, &florhit, (((long) sp->clipdist) << 2) - GETZRANGE_CLIP_ADJ, CLIPMASK_PLAYER);
    sp->cstat = save_cstat;
...
int
DoFindGroundPoint(SHORT SpriteNum)
    {
    SPRITEp sp = &sprite[SpriteNum], hsp;
    USERp u = User[SpriteNum];
    long ceilhit, florhit;
...
#if WORM == 1
int
DoBloodWorm(SHORT Weapon)
    {
    SPRITEp sp = &sprite[Weapon], exp;
    USERp u = User[Weapon];
    int offset;
    long move_ground_missile(short spritenum, long xchange, long ychange, long zchange, long ceildist, long flordist, ULONG cliptype, long numtics);
    short ang;
    long x,y,z,xvect,yvect;
    long bx,by;
    long amt;
...
MissileSetPos(short Weapon, ANIMATORp DoWeapon, long dist)
    ...
    SPRITEp wp = &sprite[Weapon];
    USERp wu = User[Weapon];
    long oldspeed, oldvel, oldzvel;
    long oldxc, oldyc, oldzc;
    char retval = FALSE;
...
BOOL
TestMissileSetPos(short Weapon, ANIMATORp DoWeapon, long dist, long zvel)
    {
    SPRITEp wp = &sprite[Weapon];
    USERp wu = User[Weapon];
    long oldspeed, oldvel, oldzvel;
...
InitMiniSumoClap(short SpriteNum)
    ...
    if (dist < CLOSE_RANGE_DIST_FUDGE(u->tgt_sp, sp, reach))
        {
        if (FAFcansee(u->tgt_sp->x,u->tgt_sp->y,SPRITEp_MID(u->tgt_sp),u->tgt_sp->sectnum,sp->x,sp->y,SPRITEp_MID(sp),sp->sectnum))
            {
            PlaySound(DIGI_30MMEXPLODE, &sp->x, &sp->y, &sp->z, v3df_none);
...
int
WeaponAutoAimZvel(SPRITEp sp, short Missile, long *zvel, short ang, BOOL test)
    {
    USERp wu = User[Missile];
    USERp u = User[sp - sprite];
    SPRITEp wp = &sprite[Missile];
...
InitStar(PLAYERp pp)
    ...
    USERp wu;
    SPRITEp wp;
    long nx, ny, nz, oldx, oldy, oldspeed;
...
int ContinueHitscan(PLAYERp pp, short sectnum, long x, long y, long z, short ang, long xvect, long yvect, long zvect)
    {
    short j;
    short hitsect, hitwall, hitsprite;
    long hitx, hity, hitz;
...
WallSpriteInsideSprite(SPRITEp wsp, SPRITEp sp)
    ...
    xoff = (long) TILE_XOFF(wsp->picnum) + (long) wsp->xoffset;
    ...
    return (clipinsideboxline(sp->x, sp->y, x1, y1, x2, y2, ((long) sp->clipdist) << 2));
    }
...
int
InitTracerTurret(short SpriteNum, short Operator, long horiz)
    {
    USERp u = User[SpriteNum];
    SPRITEp sp = u->SpriteP;
    SPRITEp wp, hsp;
...
BOOL
HitscanSpriteAdjust(short SpriteNum, short hitwall)
    {
    SPRITEp sp = &sprite[SpriteNum];
...
int
SpawnSwordSparks(PLAYERp pp, short hitsect, short hitwall, long hitx, long hity, long hitz, short hitang)
    {
    USERp u = User[pp->PlayerSprite];
    short j;
    long nx,ny;
...
int
SpawnTurretSparks(SPRITEp sp, short hitsect, short hitwall, long hitx, long hity, long hitz, short hitang)
    {
    //USERp u = User[sp - sprite];
    short j;
    long nx,ny;
...
int
SpawnShotgunSparks(PLAYERp pp, short hitsect, short hitwall, long hitx, long hity, long hitz, short hitang)
    {
    USERp u = User[pp->PlayerSprite];
    short j;
    long nx,ny;
    ...
    for (i = 0; sop->sp_num[i] != -1; i++)
        ...
        if (sprite[sop->sp_num[i]].statnum == STAT_SO_SHOOT_POINT)
            ...
            if (sop->Animator)
                ...
                // only auto aim for Z
                daang = 512;
                if ((hitsprite = WeaponAutoAimHitscan(sp, &daz, &daang, FALSE)) != -1)
                    {
...
InitEnemyUzi(short SpriteNum)
    ...
    j = SpawnSprite(STAT_MISSILE, UZI_SMOKE+2, s_UziSmoke, hitsect, hitx, hity, hitz, daang, 0);
    wp = &sprite[j];
    wp->shade = -40;
    wp->xrepeat = UZI_SMOKE_REPEAT;
    wp->yrepeat = UZI_SMOKE_REPEAT;
...
InitFireball(PLAYERp pp)
    ...
    USERp u = User[pp->PlayerSprite];
    SPRITEp wp;
    long nx = 0, ny = 0, nz, oldx, oldy, oldspeed;
    short new_angle, w, hitsprite;
    USERp wu;
    short oclipdist;
    long zvel;
...
BOOL
WarpToUnderwater(short *sectnum, long *x, long *y, long *z)
    {
    short i, nexti;
...
BOOL
WarpToSurface(short *sectnum, long *x, long *y, long *z)
    {
    short i, nexti;
    SECT_USERp sectu = SectUser[*sectnum];
    short over, under;
...
int
SpawnSplashXY(long hitx, long hity, long hitz, short sectnum)
    {
    USERp wu;
    SPRITEp wp;
    short w;
...
int QueueWallBlood(short hitsprite, short ang)
    ...
    sp->z = hitz;
    sp->shade -= 5;  // Brighten it up just a bit
    sp->yvel = hitwall; // pass hitwall in yvel
...
DoFloorBlood(short SpriteNum)
    ...
    long dist, near_dist = FEET_IN_BLOOD_DIST, a,b,c;
    short pnum;
    PLAYERp pp;
    short xsiz,ysiz;
...
SpawnShell(short SpriteNum, short ShellNum)
    {
    extern int InitShell(SHORT SpriteNum, SHORT ShellNum);
...
int
DoShrapVelocity(SHORT SpriteNum)
    {
    SPRITEp sp = &sprite[SpriteNum];
    USERp u = User[SpriteNum];
    LONG dax, day, daz;
...
//                PlaySound(DIGI_DHCLUNK, &sp->x, &sp->y, &sp->z, v3df_dontpan);
            ...
            case HIT_SECTOR:
                ...
                if (SlopeBounce(SpriteNum, &hitwall))
                    {
                    if (hitwall)
                        {
                        // hit a wall
